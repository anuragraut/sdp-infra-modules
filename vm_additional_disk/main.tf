...
resource "azurerm_managed_disk" "additional-disk01" {
    name                    = "${var.vmname}-data-disk01"
    location                = "${var.location}"
    resource_group_name     = "${var.resource_group_name}"
    storage_account_type    = "Standard_LRS"
    create_option           = "Empty"
    disk_size_gb            = "${var.additional_disk01_size}"
}
...
resource "azurerm_virtual_machine" "myterraformvm" {
    ...
    storage_data_disk {
        name              = "${var.vmname}-data-disk01"
        managed_disk_id   = "${azurerm_managed_disk.additional-disk01.id}"
        managed_disk_type = "Standard_LRS"
        disk_size_gb      = "${var.additional_disk01_size}"
        create_option     = "Attach"
        lun               = "1"
