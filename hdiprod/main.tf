...
resource "azurerm_virtual_network" "vnet02" {
  name = "${var.project}-${var.subcluster02}-${var.clusterid}-vnet"
  address_space = ["10.${var.ipid02}.1.0/25","10.${var.ipid02}.2.0/25","10.${var.ipid02}.3.0/25"]
  location = "${azurerm_resource_group.rg.location}"
  resource_group_name = "${azurerm_resource_group.rg.name}"
...
resource "azurerm_subnet" "datagatewaysubnet02" {
  name                 = "${var.project}-${var.subcluster02}-${var.clusterid}-datagateway"
  resource_group_name  = "${azurerm_resource_group.rg.name}"
  virtual_network_name = "${azurerm_virtual_network.vnet02.name}"
  address_prefix       = "10.${var.ipid02}.1.0/25"
  network_security_group_id = "${module.firewall02.id}"
...
resource "azurerm_subnet" "processingsubnet02" {
  name                 = "${var.project}-${var.subcluster02}-${var.clusterid}-processing"
  resource_group_name  = "${azurerm_resource_group.rg.name}"
  virtual_network_name = "${azurerm_virtual_network.vnet02.name}"
  address_prefix       = "10.${var.ipid02}.2.0/25"
  network_security_group_id = "${module.firewall02.id}"
...
resource "azurerm_sql_server" "sqlserver02" {
  name                = "${var.project}${var.subcluster02}${var.clusterid}sql"
  resource_group_name = "${azurerm_resource_group.rg.name}"
  location            = "${azurerm_resource_group.rg.location}"
  version             = "${var.sqlversion}"
  administrator_login = "${var.project}${var.subcluster02}_admin"
  administrator_login_password = "$dP_4dm1N"
  count               = "${var.externaldb == "yes" ? 1 : 0}"
}

resource "azurerm_sql_database" "ooziemetastore02" {
  name                = "${var.project}${var.subcluster02}${var.clusterid}ooziemetastore"
  resource_group_name = "${azurerm_resource_group.rg.name}"
  location            = "${azurerm_resource_group.rg.location}"
  server_name         = "${azurerm_sql_server.sqlserver02.name}"
}

resource "azurerm_sql_database" "hivemetastore02" {
  name                = "${var.project}${var.subcluster02}${var.clusterid}hivemetastore"
  resource_group_name = "${azurerm_resource_group.rg.name}"
  location            = "${azurerm_resource_group.rg.location}"
  server_name         = "${azurerm_sql_server.sqlserver02.name}"
}

resource "azurerm_sql_database" "rangerdb02" {
  name                = "${var.project}${var.subcluster02}${var.clusterid}rangerdb"
  resource_group_name = "${azurerm_resource_group.rg.name}"
  location            = "${azurerm_resource_group.rg.location}"
  server_name         = "${azurerm_sql_server.sqlserver02.name}"
}
...
module "firewall02" {
  source = "../firewall"
  project = "${var.project}"
  environment = "${var.subcluster02}${var.clusterid}"
  location = "${azurerm_resource_group.rg.location}"
  resource_group_name = "${azurerm_resource_group.rg.name}"
  ip_whitelist = "${var.ip_whitelist}"
}
...
output "vnet02_name" {
  value = "${azurerm_virtual_network.vnet02.name}"
}
